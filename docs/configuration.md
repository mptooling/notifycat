...
| Key | Default | Notes |
| --- | --- | --- |
| `digest.enabled` | `false` | Turns the stuck-PR digest on or off. The feature is opt-in: omitting this section — or setting only `schedule:`/`timezone:` without `enabled: true` — leaves the digest off. |
...
| `digest.country` | *(none)* | ISO 3166-1 alpha-2 country code (e.g. `DE`) selecting the public-holiday calendar the digest skips, evaluated in `digest.timezone`. Case-insensitive. Absent/empty means no holidays are skipped — weekends still are — and the server warns once at boot. An unrecognized code does **not** fail startup: the server warns, lists the supported set, and runs weekends-only. Global only — a per-repository `digest:` override may not set it. See [Weekends and holidays](digest.md#weekends-and-holidays) for the supported set and its limits. |

**Weekends are always skipped** and there is no key for it: Saturday and Sunday never get a digest, whatever `digest.schedule` says.
