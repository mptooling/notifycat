...
The server detects every retired variable listed above at startup. If any is set, startup is aborted with a message like:

```
FATAL: environment variable SLACK_REACTION_NEW_PR is no longer supported; set slack.reactions.new_pr in config.yaml instead. See docs/0.17-config-migration.md.
```

Remove the variable from your environment (`.env`, Docker env block, or deployment secret manager) and add the corresponding `config.yaml` key before restarting.
