...
const openTask = `Task: for a newly opened pull request, decide for every candidate channel (all of them — never omit one) how loud it is (ping keeps that channel's listed mentions or a subset; quiet drops them), the leading emoji (from the allowed set), the format (standard, or compact for routine low-attention changes), the emphasis (breaking only when the change is backwards-incompatible), and an optional context_block (one muted line of channel-relevant context, max 120 characters). To quiet a less-relevant channel choose quiet, never omission. Also return a one-line rationale.`
