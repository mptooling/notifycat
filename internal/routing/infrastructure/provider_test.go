...
func TestProvider_Load_MissingFile_ReturnsFileNotFoundError(t *testing.T) {
	...
	var nfe *infrastructure.FileNotFoundError
...
func TestProvider_Load_MalformedFile_ReturnsParseError(t *testing.T) {
	...
	var pe *infrastructure.ParseError
	if !errors.As(err, &pe) {
...
func TestGet_WildcardFallback(t *testing.T) {
	...
	if len(got.Mentions) != 1 || got.Mentions[0] != domain.ChannelMention {
		t.Errorf("mentions = %v; want @channel default", got.Mentions)
	}
}

func TestGet_NoOrgOrNoTier(t *testing.T) {
	p := application.NewProvider(domain.Defaults{}, map[string]domain.Org{
		"acme": {"api": {Channel: "C0API"}}, // no "*"
	}, nil)
	if _, err := p.Get(context.Background(), "acme/other"); !errors.Is(err, domain.ErrNotFound) {
		t.Errorf("err = %v; want ErrNotFound (no tier, no wildcard)", err)
	}
	if _, err := p.Get(context.Background(), "ghost/api"); !errors.Is(err, domain.ErrNotFound) {
		t.Errorf("err = %v; want ErrNotFound (no org)", err)
	}
}

func TestNewProvider_BehavesLikeLoad(t *testing.T) {
	m := map[string]domain.Org{
		"acme": {
			"web": {Channel: "C0123ABCDE", Mentions: []string{"<@U1>"}, MentionsPresent: true},
		},
	}
	p := application.NewProvider(domain.Defaults{}, m, nil)
	...
	if d := p.Digest(); !d.Enabled || d.Schedule != domain.DefaultDigestSchedule {
...
func TestGet_PopulatesResolvedBehavior(t *testing.T) {
	...
	shipit := "shipit"
	p := application.NewProvider(global, map[string]domain.Org{
		"acme": {
...
func TestDigestFor_RepoOverridesGlobal(t *testing.T) {
	...
	if !dd.Enabled || dd.Schedule != domain.DefaultDigestSchedule {
...
func TestSchedules_DistinctEnabledOnly(t *testing.T) {
	...
	p := application.NewProvider(domain.Defaults{}, map[string]domain.Org{
		"acme": {
			...
			"mute": {Channel: "C0MUTE", Digest: &domain.DigestConfig{Enabled: off}},
			"*":    {Channel: "C0DEFAULT"},
		},
	}, nil)
	got := p.Schedules()
	// weekdays + default 9am; "mute" disabled contributes nothing
	want := map[string]bool{weekdays: true, domain.DefaultDigestSchedule: true}
